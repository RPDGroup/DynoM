...
class Protenix(nn.Module):
    ...
    def __init__(self, configs) -> None:
        ...
        self.only_encoder = self.configs.only_encoder
    ...
    def get_pairformer_output(
        ...
        if self.only_encoder:
            # print("single:",type(s),"type:",s.shape)
            # # 提取数字部分并转换为NumPy数组 
            # print("z:",type(z),"type:",z.shape)
            s_data = s.to(torch.float32).cpu().numpy()
            # np.save('./L_test_output/7wux_single_data.npy', s_data)
            pair_data = z.to(torch.float32).cpu().numpy()
            # np.save('./L_test_output/7wux_pair_data.npy', pair_data)
    ...
    def main_inference_loop(
        ...
        for _ in range(N_model_seed):
            ...
            if self.only_encoder:
                s_data, pair_data = self._main_inference_loop(
    ...
    def _main_inference_loop(
        ...
        if self.only_encoder:
            s_data, pair_data = self.get_pairformer_output(
            input_feature_dict=input_feature_dict,
            N_cycle=N_cycle,
            inplace_safe=inplace_safe,
            chunk_size=chunk_size,
    ...
    def forward(
        ...
        if mode == "train":
            ...
            if self.only_encoder:
                s_data, pair_data = self.main_inference_loop(
                input_feature_dict=input_feature_dict,
                label_dict=None,
