...
### 
from protenix.data.infer_data_pipeline_pall import get_inference_dataloader
###
...
logging.basicConfig(level=logging.ERROR)  # 只显示 ERROR 级别的日志
import torch.multiprocessing as mp
os.environ["TMPDIR"] = "/home/tom/fsas/tmp_work"
...
class InferenceRunner(object):
    ...
    def predict(self, data: Mapping[str, Mapping[str, Any]]) -> dict[str, torch.Tensor]:
        ...
        with enable_amp:
            if self.configs.only_encoder:
                s_data, pair_data = self.model(
                input_feature_dict=data["input_feature_dict"],
                label_full_dict=None,
                label_dict=None,
                mode="inference",
                )
                return s_data, pair_data
            else:
...
def process_sample(data, atom_array, data_error_message, runner,configs,sum_num_data):
    try:
        ...
        logger.info(
            (
                f"[Rank {DIST_WRAPPER.rank} ({data['sample_index'] + 1}/{sum_num_data})] {sample_name}: "
            )
        )
...
def process_batch(batch, runner,configs,sum_num_data):
    # 使用 ProcessPoolExecutor 来并行处理每个样本
    with concurrent.futures.ProcessPoolExecutor() as executor:
        futures = []
        for data, atom_array, data_error_message in batch:
            futures.append(executor.submit(process_sample, data, atom_array, data_error_message, runner,configs,sum_num_data))
...
def infer_predict(runner: InferenceRunner, configs: Any) -> None:
    ...
    sum_num_data = len(dataloader.dataset)
    print("num_data:",sum_num_data)
    for seed in configs.seeds:
        seed_everything(seed=seed, deterministic=True)
        for batch in dataloader:
            process_batch(batch, runner,configs,sum_num_data)
...
def run() -> None:
    ...
    print('configs.only_encoder: ',configs.only_encoder)
    main(configs)
