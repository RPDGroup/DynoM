...
def parse(
    ...
    # 根据内部mmCIF编号方案确定蛋白质链及其起始编号（可能但不保证为1）。
    valid_mmcif_chains, entity_to_mmcif_chains = _get_protein_chains(parsed_info=parsed_info)
    # pprint.pprint(valid_mmcif_chains)
    # pprint.pprint(entity_to_mmcif_chains)
    ...
    for _, atomsite in atomsite_dict.items():
        ...
        # 筛选有效的链 ID -mmcif_to_author_chain_id

        if atomsite.mmcif_chain_id in valid_mmcif_chains:
            # assure chain id mapping consistency 验证链 ID 映射的一致性
...
def _get_protein_chains(
    ...
    """基于 _entity_poly_seq、_chem_comp、_struct_asym
    三个 section 判断哪些链是“蛋白质链”，并建立起 entity 和 chain 的对应关系。"""
