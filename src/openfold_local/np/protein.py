...
def to_pdb(prot: Protein, 
    ...
    b_factors = np.array([0]*len(aatype)*37 ) if plddt == None else plddt
    chain_index = prot.chain_index.astype(np.int32)

    # 获取每个数字第一次出现的下标
    unique_vals = np.unique(chain_index)

    # 找到每个数字第一次出现的位置
    chain_first_occurrences = [
        np.where(chain_index == val)[0][0] for val in unique_vals
    ]
    ...
    for i in range(aatype.shape[0]):
        ...
        for atom_name, pos, mask in zip(
            ...
            atom_line = (
                ...
                f"{residue_index[i]-chain_first_occurrences[chain_index[i]]:>4}{insertion_code:>1}   "
                f"{pos[0]:>8.3f}{pos[1]:>8.3f}{pos[2]:>8.3f}"
                f"{occupancy:>6.2f}{b_factors[b_factor_line]:>6.2f}          "
        ...
        if should_terminate:
            ...
            chain_termination_line = (
                ...
                f"{chain_tag:>1}{residue_index[i]-chain_first_occurrences[chain_index[i]]:>4}"
            )
