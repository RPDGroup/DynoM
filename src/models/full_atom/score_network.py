...
from src.openfold_local.config import model_config
import src.openfold_local.utils.loss as af2_loss
from tqdm import tqdm


from .model_nn.base import FoldNet
from .diffuser.se3_diffuser import SE3Diffuser
from .utils.all_atom import atom14_to_atom37
from .utils.clash_checker import get_atom_radius,between_residue_clash_loss
...
class BaseScoreNetwork(nn.Module):
    def __init__(
        ...
        self.model_nn = FoldNet(embedder_cfg=model_nn_cfg['embedder'],
                                structure_module_cfg=model_nn_cfg['structure_module'],
                                confidence_head_cfg=model_nn_cfg.get('confidence_head', None))
        self.diffuser = SE3Diffuser(se3_cfg=se3_cfg)
        self.loss_cfg = loss_cfg
        self.reverse_sample_cfg = reverse_sample_cfg
        self.is_show_diffusing = reverse_sample_cfg['is_show_diffusing']
    ...
    def forward(
        ...
        loss, aux_info = self.loss_fn(
            ...
            rigids_mask=rigids_mask,
            pred_rigids_0=model_out["pred_rigids_0"],
            pred_torsions=model_out["pred_torsions"],
            pred_atom14=model_out["pred_atom14"],
            # confidence_score=model_out["confidence_scores"],
            pred_rot_score=pred_rot_score,
            pred_trans_score=pred_trans_score,
            pred_sidechain_frame=model_out["pred_sidechain_frames"],
            gt_feat=gt_feat,
        )
    ...
    def loss_fn(
        ...
        rot_loss_weight = self.loss_cfg['rot_loss_weight']
        rot_angle_loss_t_filter = self.loss_cfg['rot_angle_loss_t_filter']
        trans_loss_weight = self.loss_cfg['trans_loss_weight']
        bb_coords_loss_weight = self.loss_cfg['bb_coords_loss_weight']
        bb_coords_loss_t_filter = self.loss_cfg['bb_coords_loss_t_filter']
        bb_dist_map_loss_weight = self.loss_cfg['bb_dist_map_loss_weight']
        torsion_loss_weight = self.loss_cfg['torsion_loss_weight']
        bb_dist_map_loss_t_filter = self.loss_cfg['bb_dist_map_loss_t_filter']
        fape_loss_weight = self.loss_cfg['fape_loss_weight']
        clash_loss_weight = self.loss_cfg['clash_loss_weight']
        ...
        atom14_atom_radius = get_atom_radius(atom14_pred_positions=pred_atom14, 
                                             aatype=aatype,
                                             atom14_atom_exists=atom14_atom_exists)  # register atom radius
        ...
        if bb_coords_loss_weight + bb_dist_map_loss_weight > 1e-4:
            ...
            ) / (bb_coords_mask.sum(dim=(-2, -1)) + 1e-6)  # (B,)
        ...
        if fape_loss_weight > 0:
            ...
            fape_loss = torch.zeros_like(input=trans_loss)
        ...
        loss = (
                ...
                + clash_loss_weight * between_residue_clashes['mean_loss']
                )
        ...
        clash_loss = between_residue_clashes['mean_loss'].mean()
    ...
    def reverse_sample(
        ...
        dt = torch.Tensor([1.0 / self.reverse_sample_cfg['diffusion_steps']] * batch_size).to(
            aatype.device
        )  # (B,)
        ...
        rigids_t = self.diffuser.sample_ref(
            ...
            temperature=self.reverse_sample_cfg['temperature'],
        )  # (B, L, 7)

        step_diffusion = []
        print(f"Diffusing for {self.reverse_sample_cfg['diffusion_steps']} steps...")

        for step_t in tqdm(range(self.reverse_sample_cfg['diffusion_steps']), disable=False):
            model_out = self.model_nn(
                aatype=aatype,
                chain_ids=chain_ids,
                padding_mask=padding_mask,
            ...
            if t.min() < 0.01:
                break
            if self.is_show_diffusing:
                step_pred_atom37, step_atom37_mask = atom14_to_atom37(model_out["pred_atom14"], aatype)
                step_diffusion.append({
                    'step_idx':step_t,
                    'atom37': step_pred_atom37,
                    'atom37_mask':step_atom37_mask
                })

        pred_atom37, atom37_mask = atom14_to_atom37(model_out["pred_atom14"], aatype)
        
        return {"atom37": pred_atom37, 
                "atom37_mask": atom37_mask, 
                "confidence_score": model_out.get("confidence_scores", None),
                }, step_diffusion
